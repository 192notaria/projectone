                    ...
                    <div class="container">
                        <div class="row">
                            <div class="col-lg-12">
                                ...
                                <input wire:model='nombre' type="text" class="form-control" placeholder="Morelia, Uruapan, Zamora...">
                                @error('nombre') <span class="mt-3 badge badge-danger">{{ $message }}</span>@enderror
                            </div>
                            <div class="col-lg-12 mt-4">
                                <div class="form-group autocomplete">
                                    <label for="">Asignar Estado</label>
                                    <input type="text" class="form-control" wire:model="buscarEstado" placeholder="Michoacan, Jalisco, Sonora...">
                                    @error('buscarEstado') <span class="text-danger">{{ $message }}</span>@enderror
                                    <div class="autocomplete-items">
                                        @foreach ($estados as $estado)
                                            <div>
                                                <a wire:click='selectEstado({{$estado->id}})'>
                            ...
                            <div class="col-lg-12 mt-3">
                                @if ($estado_id == "")
                                    <span style="width: 100%;" class="badge badge-danger">Sin estado seleccionado</span>
                                @else
                                    <span style="width: 100%;" class="badge badge-success">{{$estado_nombre}}</span>
                                @endif
                            </div>
                        </div>
                    </div>
